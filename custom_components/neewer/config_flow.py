...
class NeewerLightConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_bluetooth(
        ...
        _LOGGER.debug(
            ...
            discovery_info.name, discovery_info.address, discovery_info.connectable
        )
        _LOGGER.debug(
            "[BLUETOOTH DISCOVERY] Device details - RSSI: %s, Service UUIDs: %s, Manufacturer data: %s",
            discovery_info.rssi, discovery_info.service_uuids, discovery_info.manufacturer_data
        )
        
        await self.async_set_unique_id(discovery_info.address)
        _LOGGER.debug("[BLUETOOTH DISCOVERY] Set unique_id to: %s", discovery_info.address)
        self._abort_if_unique_id_configured()
        _LOGGER.debug("[BLUETOOTH DISCOVERY] Unique ID check passed, device not already configured")
        ...
        _LOGGER.debug(
            ...
            discovery_info.name, is_neewer
        )
        if not is_neewer:
            _LOGGER.debug("[BLUETOOTH DISCOVERY] Aborting - device name doesn't match Neewer patterns")
            return self.async_abort(reason="not_neewer_light")

        self._discovered_device = discovery_info
        _LOGGER.debug("[BLUETOOTH DISCOVERY] Device validated, proceeding to confirmation step")
    ...
    async def async_step_bluetooth_confirm(
        ...
        _LOGGER.debug(
            ...
            user_input is not None, self._discovered_device.name if self._discovered_device else None
        )
    ...
    async def async_step_user(
        ...
        _LOGGER.debug("[USER DISCOVERY] Manual discovery started, user_input: %s", user_input is not None)
        ...
        if user_input is not None:
            _LOGGER.debug("[USER DISCOVERY] User input received, proceeding with discovery")
        ...
        _LOGGER.debug("[USER DISCOVERY] Current configured addresses: %s", current_addresses)
        
        discovered_devices = list(async_discovered_service_info(self.hass, connectable=False))
        _LOGGER.debug("[USER DISCOVERY] Found %d total Bluetooth devices", len(discovered_devices))
        
        for discovery_info in discovered_devices:
            _LOGGER.debug(
                "[USER DISCOVERY] Checking device - Name: '%s', Address: %s, Already configured: %s",
                discovery_info.name, discovery_info.address, discovery_info.address in current_addresses
            )
            ...
            if discovery_info.address not in current_addresses:
                ...
                if is_neewer:
                    _LOGGER.debug("[USER DISCOVERY] Found suitable Neewer device, setting up config")
                    await self.async_set_unique_id(discovery_info.address)
                    _LOGGER.debug("[USER DISCOVERY] Set unique_id to: %s", discovery_info.address)
                    self._abort_if_unique_id_configured()
